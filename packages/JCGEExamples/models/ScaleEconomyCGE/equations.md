...
- `prod.eqpzs[BRD]` pz[i] = ay[i] * py[i] + sum_j∈{BRD, MLK}(ax[j,i] * pq[j]) + FC[i] / Z[i]
...
- `prod.eqpzs[MLK]` pz[i] = ay[i] * py[i] + sum_j∈{BRD, MLK}(ax[j,i] * pq[j]) + FC[i] / Z[i]
- `factor_market.eqF[CAP]` sum_j∈{BRD, MLK}(F[h,j]) = FF[h]
- `factor_market.eqF[LAB]` sum_j∈{BRD, MLK}(F[h,j]) = FF[h]
- `government.eqTd` Td = tau_d * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + 0.0 + sum_i∈{BRD, MLK}(FC[i])
...
- `private_saving.eqSp` Sp = ssp * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + 0.0 + sum_i∈{BRD, MLK}(FC[i])
- `investment.eqXv[BRD]` Xv[i] = lambda[i] * Sp + Sg + epsilon * Sf / pq[i]
- `investment.eqXv[MLK]` Xv[i] = lambda[i] * Sp + Sg + epsilon * Sf / pq[i]
- `household.eqXp[BRD]` Xp[i] = alpha[i] * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + -Sp + -Td + 0.0 + sum_j∈{BRD, MLK}(FC[j]) / pq[i]
- `household.eqXp[MLK]` Xp[i] = alpha[i] * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + -Sp + -Td + 0.0 + sum_j∈{BRD, MLK}(FC[j]) / pq[i]
