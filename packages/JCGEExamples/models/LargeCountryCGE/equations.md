...
- `government.eqTd` Td = tau_d * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + 0.0 + 0.0
...
- `private_saving.eqSp` Sp = ssp * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + 0.0 + 0.0
- `investment.eqXv[BRD]` Xv[i] = lambda[i] * Sp + Sg + epsilon * Sf / pq[i]
- `investment.eqXv[MLK]` Xv[i] = lambda[i] * Sp + Sg + epsilon * Sf / pq[i]
- `household.eqXp[BRD]` Xp[i] = alpha[i] * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + -Sp + -Td + 0.0 + 0.0 / pq[i]
- `household.eqXp[MLK]` Xp[i] = alpha[i] * sum_h∈{CAP, LAB}(pf[h] * FF[h]) + -Sp + -Td + 0.0 + 0.0 / pq[i]
...
- `foreign_trade.eqfe[BRD]` E[i] / E0[i] = pWe[i] / pWe0[i]^-sigma[i]
- `foreign_trade.eqfm[BRD]` M[i] / M0[i] = pWm[i] / pWm0[i]^psi[i]
- `foreign_trade.eqfe[MLK]` E[i] / E0[i] = pWe[i] / pWe0[i]^-sigma[i]
- `foreign_trade.eqfm[MLK]` M[i] / M0[i] = pWm[i] / pWm0[i]^psi[i]
- `armington.eqQ[BRD]` Q[i] = gamma[i] * delta_m[i] * M[i]^eta[i] + delta_d[i] * D[i]^eta[i]^1.0 / eta[i]
- `armington.eqM[BRD]` M[i] = gamma[i]^eta[i] * delta_m[i] * pq[i] / 1.0 + 0.0 + tau_m[i] * pm[i]^1.0 / 1.0 + -eta[i] * Q[i]
...
- `armington.eqM[MLK]` M[i] = gamma[i]^eta[i] * delta_m[i] * pq[i] / 1.0 + 0.0 + tau_m[i] * pm[i]^1.0 / 1.0 + -eta[i] * Q[i]
